// <copyright file="ReflectionRemoting.cs" company="Drastic Actions">
...
namespace Drastic.Diagnostics.Representations.Reflection
{
    ...
    public class CSharpWriter : IReflectionRemotingVisitor
    {
        ...
        public virtual void VisitStackFrame (StackFrame stackFrame)
        {
            ...
            if (stackFrame.Member != null) {
                ...
                writer.Write ("(wrapper {0}) ", stackFrame.InternalMethod.WrapperType ?? string.Empty);
        ...
        public virtual void VisitParameter (Parameter parameter)
        {
            if (parameter.IsOut) {
                ...
                writer.Write (' ');
            } else if (parameter.Type is not null && parameter.Type.IsByRef ()) {
                ...
                foreach (var name in typeSpec.GetAllNames())
                {
                    ...
                    if (name.TypeArgumentCount > 0)
                    {
                        ...
                        for (var i = 0; i < name.TypeArgumentCount; i++, typeArgIndex++)
                        {
                            ...
                            if (typeSpec?.TypeArguments is not null)
    ...
    public sealed class TypeNode : Node, ISerializable
    {
    ...
    public sealed class ExceptionNode : Node, ISerializable
    {
...
#nullable disable
    public sealed class StackTrace : Node, ISerializable
    {
...
#nullable enable
    ...
    public sealed class StackFrame : Node, ISerializable
    {
        ...
        public static StackFrame? Create (System.Diagnostics.StackFrame frame)
        {
            ...
            if (method != null) {
                isTaskAwaiter = (method?.DeclaringType?.IsTaskAwaiter() ?? false) || (method?.DeclaringType?.DeclaringType?.IsTaskAwaiter() ?? false);
    ...
    public sealed class Parameter : Node, ISerializable
    {
    ...
    public sealed class Method : Node, ITypeMember, ISerializable
    {
        public string? Name { get; }
        ...
        public static Method? Create (MethodBase? method)
        {
            ...
            var result = method.GetParameters()?.Select(Parameter.Create).ToListOrNullIfEmpty();
    ...
    public sealed class Field : Node, ITypeMember, ISerializable
    {
        public string Name { get; }
        public TypeSpec DeclaringType { get; }
    ...
    public sealed class Property : Node, ITypeMember, ISerializable
    {
        public string Name { get; }
        public TypeSpec DeclaringType { get; }
