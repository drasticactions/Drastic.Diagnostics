// <copyright file="TypeSpec.cs" company="Drastic Actions">
    ...
    public sealed class TypeSpec
    {
        public struct TypeName : IEquatable<TypeName>
        {
            ...
            public static TypeName Parse (string? @namespace, string name)
            {
                ...
                return new TypeName (@namespace ?? string.Empty, name, arity);
            }
        }
        ...
        static readonly PropertyInfo? IsVariableBoundArray = typeof (Type).GetProperty (nameof (IsVariableBoundArray));
        ...
        public static TypeSpec Create (Type? type)
        {
            ...
            while (type!.HasElementType) {
                ...
                if (type.IsByRef)
                    ...
                    var result = IsVariableBoundArray?.GetValue(type);
                    if (rank == 1 && (result is not null && (bool)result))
